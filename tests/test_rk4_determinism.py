...
def test_rk4_rollout_bitwise_identical(model: NLinkCartPole) -> None:
    ...
    t1, x1, u1, ur1 = model.rollout_zoh(
        x0,
        policy,
        t_final=0.5,
    ...
    t2, x2, u2, ur2 = model.rollout_zoh(
        x0,
        policy,
        t_final=0.5,
        control_dt=spec.control_dt_s,
    ...
    assert np.array_equal(ur1, ur2)  # raw demanded force also deterministic
