...
Locked weights: ``Q`` diag cart_pos 1.0 | each angle 80.0 | cart_vel 1.0 |
each ang_vel 5.0, ``R = [[0.02]]``. All linearization comes from the shared
dynamics module; no EOM is duplicated here.
"""
...
def wrap_state_error(x: np.ndarray, x_ref: np.ndarray, n_links: int) -> np.ndarray:
    """State error ``x - x_ref`` with the angle components wrapped to pi.

    State layout is ``[x_cart, theta_1..theta_n, xdot, thetad_1..thetad_n]``.
...
def make_Q(n_links: int) -> np.ndarray:
    """Build the locked diagonal state-cost matrix ``Q``.
...
def make_R() -> np.ndarray:
    """Return the locked control-cost matrix ``R = [[0.02]]``."""
...
def static_lqr(
    ...
    Returns ``(K, P)`` where ``K`` has shape ``(1, nx)`` (gain) and ``P`` is
    the ``(nx, nx)`` Riccati solution defining ``V(x) = e'P e``.
    """
    n = model.n
    if Q is None:
        Q = make_Q(n)
