"""Frozen physical and timing specification for the n-link cart-pole.

Single home of the spec. ``dynamics.py`` consumes a :class:`CartPoleSpec`
instance. Coordinate convention: ``q = [x_cart, theta_1..theta_n]``,
``state = [q, qdot]`` of length ``2 * (n_links + 1)``. ``theta_i = 0`` means
link ``i`` points up, ``theta_i = pi`` means down. Angles are absolute world
angles. All per-link lists must have exactly ``n_links`` entries.
"""
...
class CartPoleSpec(BaseModel):
    """Immutable physical and timing specification for an n-link cart-pole."""
    ...
    def _check_list_lengths(self) -> CartPoleSpec:
        ...
        for name in ("link_masses_kg", "link_lengths_m", "damping_links_n_m_s_rad"):
            ...
            if len(vals) != n:
                raise ValueError(f"{name} has length {len(vals)}, expected n_links={n}")
    ...
    def with_n_links(self, n: int) -> CartPoleSpec:
        ...
        Per-link mass, length, and damping are taken from the first entry of
        the current spec and broadcast across ``n`` links.
        """
        return CartPoleSpec(
            n_links=n,
...
# Mapping from YAML field names to CartPoleSpec field names.
_YAML_ALIASES = {
    "n_links": "n_links",
...
# Keys that appear in capsule YAML configs but do not belong to the dynamics
# spec (they govern controllers and proof). Silently ignored.
_IGNORED_YAML_KEYS = {
    "link_inertia",
    "logging_dt_s",
    "hold_time_s",
...
def load_spec(path: str | Path) -> CartPoleSpec:
    ...
    ignored. Raises ``ValueError`` on any other unrecognized key.
    """
