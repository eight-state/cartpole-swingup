"""Tier-0 fast replacements for gate Python loops (bit-exact targets).

``make_densifier``: mapaccum fold of the gate's ``densify()`` (stride x n_sub
RK4 substeps per coarse node). The graph composes the same ``model.f`` calls
in the same order as ``NLinkCartPole.rk4_step`` so results are bit-identical.
``FastDTVLQR``: :class:`~cartpole_capsules.core.discrete_tvlqr.DiscreteTVLQR`
with the per-tick zoh loop replaced by one batched CasADi call for all
(A, B) pairs, then the same scipy ``expm`` and backward Riccati code path.
"""
...
def make_densifier(model, control_dt: float, n_sub: int, stride: int, n_coarse: int):
    """Build the mapaccum densifier once. Returns densify(Xp, Up) -> (Xd, Ud).

    Reproduces exactly:
        for k: xx = Xp[k]; repeat stride times { n_sub RK4 substeps of u_k };
        appending the state after each stride-group of substeps.
    Per coarse node k there are ``stride`` dense states, integrated from the
    COARSE node state ``Xp[k]`` (not chained across nodes), matching the
    gate's densify().
    """
    nx = model.nx
    ...
    def densify(Xp: np.ndarray, Up: np.ndarray):
        N = len(Up)
        assert N == n_coarse
        out = np.asarray(node_map(np.asarray(Xp)[:N].T, np.asarray(Up).reshape(1, N)))
...
def check_bitexact_densify(model, densify_fast, Xp, Up, control_dt, n_sub, stride, n_check=10):
    """Compare against the Python-loop densify on the first ``n_check`` nodes.

    Returns ``(bit_identical, max_abs_diff)``.
    """
    dt_sub = control_dt / n_sub
    Xd_f, _Ud_f = densify_fast(Xp, Up)
    md = 0.0
    same = True
    idx = 1
...
class FastDTVLQR:
    ...
    def __init__(self, model, X, U, dt, Qf=None, Q=None, R=None):
        ...
        # Batched continuous linearization (identical compiled functions).
