...
shared dynamics linearization. ``Q_tvlqr = Q_static``, ``R_tvlqr = [[0.02]]``.

Terminal cost note: the validated swing-up path passes ``Qf = P_static``
(the infinite-horizon CARE solution at upright, scale 1) so the time-varying
gains converge to the steady-state upright LQR at ``t = T``. The
``25 * P_static`` terminal weight is the default only of
:func:`build_upright_tvlqr` (the constant-upright catch helper), which is not
on the validated swing-up path.

The nominal trajectory is supplied as time-sampled ``(t, x_nom, u_nom)`` and
interpolated to evaluate ``A(t), B(t)`` at arbitrary Riccati integration
times. The construction supports any feasible nominal.
"""
...
class TVLQR:
    ...
    def _nom_at(self, t: float) -> tuple[np.ndarray, float]:
        ...
        # Constant-nominal fast path (upright catch): both endpoints equal.
...
def build_upright_tvlqr(
    ...
    Not the validated swing-up path. This is the constant-upright catch
    helper; the headline swing-up runs build TVLQR along the nominal with
    ``Qf = P_static`` (scale 1). The nominal is the upright equilibrium
    (``x_nom = x_up``, ``u_nom = 0``) over ``[0, horizon]`` and
    ``Qf = qf_scale * P_static`` (default 25, used only for this helper).
    """
