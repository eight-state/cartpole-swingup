...
def simulate_handoff(
    ...
    t_log, x_log, u_log, u_raw_log = model.rollout_zoh(
        x0, pol, total_t, control_dt, rk4_max
    )
    ...
    max_force_demanded = (
        float(np.max(np.abs(u_raw_log))) if len(u_raw_log) else 0.0
    )
    saturated = bool(
        len(u_raw_log) and np.any(np.abs(u_raw_log) > fbound + 1e-6)
    )
...
def static_hold_rollout(
    ...
    t_log, x_log, u_log, _u_raw_log = model.rollout_zoh(
