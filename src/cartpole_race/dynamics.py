...
class NLinkCartPole:
    ...
    def rollout_zoh(
        ...
        Returns:
            ``(t_log, x_log, u_log, u_raw_log)`` where ``t_log`` has shape
            ``(K+1,)``, ``x_log`` has shape ``(K+1, nx)`` (state at each control
            tick boundary, including the initial state), ``u_log`` has shape
            ``(K,)`` (the CLIPPED force actually applied during each tick), and
            ``u_raw_log`` has shape ``(K,)`` (the RAW demanded force from the
            policy before saturation, for measuring how hard the controller
            pushed against the bound).
        """
        ...
        u_raw_log = np.empty(n_ticks, dtype=float)
