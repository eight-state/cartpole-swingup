"""Committed checks on the shipped n=8 dense nominal's rigor facts.

The n=8 dense nominal (``results/nom_n8_dense1ms.npz``) is the 1 ms
...
The committed claims tested here:
  ...
  1. intra-segment ticks reproduce exactly under the simulator's ZOH step
     (defect < 1e-10),
  2. node-boundary seams < 5e-3 state units (measured max 4.233e-3; absorbed by feedback — closed loop passes at 23.2 N peak demand),
  3. the 4 ms parent solve's own RK4-4ms transcription defect < 1e-8
     (measured 2.1e-12),
  4. peak feedforward force < 30 N (measured 23.2 N) — 5x margin to the
     150 N bound.
"""
...
DENSE = REPO / "results" / "nom_n8_dense1ms.npz"
PARENT = REPO / "results" / "nom_n8_4ms.npz"
...
def test_dense_nominal_seams_bounded() -> None:
    """4 ms node-boundary seams stay below the committed 5e-3 bound."""
    d = np.load(DENSE)
    X = np.asarray(d["x"], float)
    U = np.asarray(d["u"], float).reshape(-1)
    model, spec = _model()
    ...
    # n=8 measured max seam: 4.233e-3 (vs 8.34e-5 at n=7) — the n=8
    # trajectory is stiffer so the RK4-4ms vs exact-ZOH mismatch is larger.
    # The operative gate is the closed loop, which passes with peak demand
    # 23.2 N (seams are absorbed by the per-tick discrete TVLQR).
    assert worst < 5e-3, f"node seam too large: {worst:.3e}"


def test_parent_solve_transcription_defect() -> None:
    """The 4 ms parent satisfies its own RK4-4ms transcription to < 1e-8.

    This is a SAMPLED spot-check (every 7th node) for speed, not an all-node
    sweep; the full-grid defect (2.1e-12) is reported by reproduce_n8.py.
    """
    d = np.load(PARENT)
    X = np.asarray(d["x"], float)
    U = np.asarray(d["u"], float).reshape(-1)
