...
Per the proposal 'Controllers and handoff -> Finite-horizon TVLQR catch':
    ...
    ``Q_tvlqr = Q_static``, ``R_tvlqr = [[0.02]]``.

Terminal cost (``Qf``) note: the VALIDATED swing-up path
(``r2_validate.build_tvlqr_along_nominal`` and ``reproduce_n5.py``) passes
``Qf = P_static`` (the infinite-horizon CARE solution at upright, scale 1), so
the time-varying gains converge to the steady-state upright LQR at ``t = T``.
That ``Qf = P_static`` is what produced every committed n=5 result. The
``25 * P_static`` terminal weight is the default ONLY of the
``build_upright_tvlqr`` helper (the M1 constant-upright catch), which is **not**
on the validated swing-up path; do not read the 25x scale into the headline runs.
...
def build_upright_tvlqr(
    ...
    NOT the validated swing-up path. This is the M1 constant-upright catch
    helper; the headline n=5 swing-up runs build TVLQR via
    ``r2_validate.build_tvlqr_along_nominal`` with ``Qf = P_static`` (scale 1).

    The M1 catch nominal is the upright equilibrium (``x_nom = x_up``,
    ``u_nom = 0``) over ``[0, horizon]``; ``Qf = qf_scale * P_static`` (default
    25x here, used only for this constant-upright helper).
