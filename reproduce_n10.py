"""ONE-COMMAND reproduction of the headline n=10 (decuple) cart-pole result.

    uv run python reproduce_n10.py            # fast: unperturbed pass + rho
    uv run python reproduce_n10.py --gate     # full 24-IC preroll gate, 3 seeds

Fast mode (~1 min):
  1. Loads the shipped n=10 dense nominal (configs/nominal.py).
  2. Verifies the nominal's grid facts (10000 ticks, 10 s) and reports its
     4 ms parent's RK4-4ms transcription defect (expected ~1.4e-7).
  3. Builds exact-ZOH DISCRETE-time TVLQR along the dense nominal and reports
     the closed-loop monodromy spectral radius rho (expected ~0.1042 < 1).
  4. Runs the UNPERTURBED closed loop in the real saturated simulator
     ...
     10 s window. Success is predicate v1: a continuous >= 5 s in-success-set
     run. Expected: PASS, swing peak ~36 N, handoff ~0.0115 deg, gentler
...
Gate mode (--gate): the PRE-ROLL perturbed-IC gate at sigma=0.02 (identical
  ...
  results/gate_n10_preroll_seed*.json. This reuses the SAME preroll architecture
  as n=9 with zero re-tuning.
...
def main() -> int:
    ...
    rho = tv.monodromy()
    print(f"    closed-loop monodromy rho = {rho:.4g}  (expected ~0.1042 < 1)")
    ...
    if not (ok and rho < 1.0):
        ...
        return 1
    print("\n*** n=10 SWING-UP + BALANCE: UNPERTURBED CLOSED-LOOP PASS ***")

    if "--gate" in sys.argv:
        import subprocess
        workers = str(max(1, (os.cpu_count() or 2) - 1))
        env = dict(os.environ, NLINKS="10", NOM_PATH=str(NOMINAL.path))
        rc = 0
        for seed in ("12345", "777", "2024"):
            print(f"\n[4] preroll gate (24 ICs, sigma=0.02, seed {seed}) — "
                  f"no per-IC NLP, ~3.5 min/seed ...")
    ...
    print("\n(run with --gate to regenerate the full 24-IC preroll gate on all "
          ...
          "results/gate_n10_preroll_seed*.json)")
    return 0
