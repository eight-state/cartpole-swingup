...
| "The quintuple inverted pendulum" YouTube video, id `WwR92kx6tcA` | Cart or rotary (unverified; description only) | n=5 | Claims swing-up **and** stabilization | Video (Matlab/Simulink, per description) | No | A quintuple-pendulum video exists. It ships no code, and cart-vs-rotary and swing-up-vs-balance could not be verified (the hosting platform is one the author cannot open; the claim rests on the title and description only). It is not reproducible. |
