"""Clean, reproducible demo: closed-loop swing-up + balance of the n=10
(DECUPLE) inverted pendulum on a cart, in the REAL saturated simulator —
rendered to results/demo_decuple.gif.

    uv run python scripts/demo_decuple.py

One deterministic UNPERTURBED rollout: exact-ZOH discrete TVLQR along the
shipped dense nominal over [0, 10 s], handoff to the static LQR, then hold.
The hold is watched over a 10 s window; the n=10 catch is GENTLER than n=9's
(the static-LQR hold recovers un-saturated, peak ~98.6 N, no 150 N clip).
Success is the SAME predicate the release gate uses: a continuous >= 5 s
in-success-set run (predicate v1). The GIF is asserted PASS before saving
(never a failed run). No re-solving, no ensemble, no pre-roll (the pre-roll is
only for perturbed ICs; see scripts/gate_preroll.py).
"""
...
N_LINKS = 10
HOLD_S = 5.0
HOLD_WIN = 10.0  # watch the hold over a 10 s window
GIF_NAME = "demo_decuple.gif"
...
def main() -> int:
    ...
    print(f"[demo n={n}] longest hold {max(0, best - 1) * dt:.1f} s, peak force "
