"""Clean, reproducible demo: closed-loop swing-up + balance of the n=5
(QUINTUPLE) inverted pendulum on a cart, in the REAL saturated simulator.
...
What it does
------------
1. Loads the validated n=5 nominal ``results/nom_n5_gluck_cont.npz`` (a 6.0 s
   Glueck-inversion + collocation-polish swing-up).
...
3. Runs ONE closed-loop rollout in the real saturated sim
   (``dynamics.rollout_zoh``): TVLQR feedback over [0, 6.0 s] hands off to the
...
4. Asserts SUCCESS: every link stays continuously inside the LOCKED success set
   (|theta|<=5deg, |thetadot|<=0.5, |x|<=2, |xdot|<=0.5) for the final 5 s,
   the cart stays on the track, and the force never exceeds the bound.
5. Saves the animated GIF (and plots if matplotlib's writer is available) to
   ``results/``.

Nothing here re-solves a trajectory or runs the perturbed-IC ensemble; this is
a single light rollout (one swing-up + a 5 s hold) plus plotting.
"""
...
N_LINKS = 5
HOLD_S = 5.0
FORCE_BOUND = 60.0  # matches the closed-loop validation bound; peak use is ~20 N
# Repo-relative results dir (this script lives in <repo>/scripts/).
OUT = Path(__file__).resolve().parent.parent / "results"
NOM_PATH = OUT / "nom_n5_gluck_cont.npz"
...
def run_demo(make_plots: bool = True, make_animation: bool = True):
    ...
    # --- load the validated nominal -------------------------------------------
    d = np.load(NOM_PATH)
    x_nom = d["x"]                      # (N+1, nx)
    u_nom = d["u"]                      # (N,)
    horizon = float(d["horizon"])      # 6.0 s
    ...
    print(f"[demo] n={n} nominal: {NOM_PATH.name}  horizon={horizon:.2f}s  "
          f"nodes={len(u_nom)}  force_bound={FORCE_BOUND:.0f}N", flush=True)
    ...
    total = horizon + HOLD_S
    t_log, x_log, u_log, _u_raw = model.rollout_zoh(
        x0, policy, total, control_dt, rk4)
    print(f"[demo] rollout: {len(u_log)} ticks over {total:.2f}s "
          f"(swing-up {horizon:.1f}s + hold {HOLD_S:.1f}s)", flush=True)
    ...
    print("\n[demo] SUCCESS: quintuple swung up and held upright for "
...
def _save_plots(t_log, x_log, u_log, n, horizon, force_bound):
    ...
    # (a) link angles
    ax = axes[0]
    colors = ["#1b6ca8", "#e07a5f", "#3d9970"]
    for i in range(n):
        ax.plot(t_log, ang_deg[:, i], color=colors[i % 3], lw=1.4,
                label=f"link {i + 1}")
    ...
    ax.set_title("Quintuple inverted pendulum on a cart: closed-loop "
                 "swing-up + 5 s balance (n=5, ~20 N peak / 60 N bound)")
    ...
    fig.tight_layout()
    path = OUT / "demo_quintuple_plots.png"
    ...
    fig2.tight_layout()
    path2 = OUT / "demo_quintuple_force_zoom.png"
...
def _save_animation(t_log, x_log, model, horizon):
    """Light matplotlib GIF of the quintuple swinging up and balancing.
    ...
    ax.set_title("Quintuple inverted pendulum: closed-loop swing-up + balance")
